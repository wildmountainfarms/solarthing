...
public class RoverModbusSlaveRead implements RoverReadTable {
	...
	public RoverModbusSlaveRead(int address, ModbusSlave modbus) {
	...
	private int[] get(MessageHandler<int[]> readHandler){
		return modbus.sendMessage(address, readHandler);
	}
	...
	private static MessageHandler<int[]> MAX_VOLTAGE_AND_CHARGING = new ReadRegistersHandler(0x000A, 1);
	@Override
	...
	private static MessageHandler<int[]> DISCHARGING_AND_PRODUCT_TYPE = new ReadRegistersHandler(0x000B, 1);
	...
	private static MessageHandler<int[]> PRODUCT_MODEL = new ReadRegistersHandler(0x000C, 8);
	...
	private static MessageHandler<int[]> SOFTWARE_VERSION = new ReadRegistersHandler(0x0014, 2);
	@Override public int getSoftwareVersionValue() {
		return twoRegistersAsInt(SOFTWARE_VERSION);
	}
	private static MessageHandler<int[]> HARDWARE_VERSION = new ReadRegistersHandler(0x0016, 2);
	@Override public int getHardwareVersionValue() {
		return twoRegistersAsInt(HARDWARE_VERSION);
	}
	private static MessageHandler<int[]> PRODUCT_SERIAL = new ReadRegistersHandler(0x0018, 2);
	@Override public int getProductSerialNumber() {
		return twoRegistersAsInt(PRODUCT_SERIAL);
	}
	
	private static MessageHandler<int[]> ADDRESS = new ReadRegistersHandler(0x001A, 1);
	@Override
	public int getControllerDeviceAddress() {
		return oneRegister(ADDRESS);
	}
	
	private static MessageHandler<int[]> SOC = new ReadRegistersHandler(0x0100, 1);
	@Override
	public int getBatteryCapacitySOC() {
		return oneRegister(SOC);
	}
	
	private static MessageHandler<int[]> BATTERY_VOLTAGE = new ReadRegistersHandler(0x0101, 1);
	@Override
	public float getBatteryVoltage() {
		return oneRegister(BATTERY_VOLTAGE) / 10.0F;
	}
	
	private static MessageHandler<int[]> CHARGING_CURRENT = new ReadRegistersHandler(0x0102, 1);
	@Override
	...
	private static MessageHandler<int[]> CONTROLLER_BATTERY_TEMPERATURE = new ReadRegistersHandler(0x0103, 1);
	...
	private static MessageHandler<int[]> LOAD_VOLTAGE = new ReadRegistersHandler(0x0104, 1);
	@Override
	public float getLoadVoltage() {
		return oneRegister(LOAD_VOLTAGE) / 10.0F;
	}
	
	private static MessageHandler<int[]> LOAD_CURRENT = new ReadRegistersHandler(0x0105, 1);
	@Override public float getLoadCurrent() {
		return oneRegister(LOAD_CURRENT) / 100.0F;
	}
	
	private static MessageHandler<int[]> LOAD_POWER = new ReadRegistersHandler(0x0106, 1);
	@Override public int getLoadPower() {
		return oneRegister(LOAD_POWER);
	}
	
	private static MessageHandler<int[]> PV_VOLTAGE = new ReadRegistersHandler(0x0107, 1);
	@Override public Float getInputVoltage() { // pv voltage/solar panel voltage
		return oneRegister(PV_VOLTAGE) / 10.0F;
	}
	private static MessageHandler<int[]> PV_CURRENT = new ReadRegistersHandler(0x0108, 1);
	@Override public Float getPVCurrent() {
		return oneRegister(PV_CURRENT) / 100.0F;
	}
	
	private static MessageHandler<int[]> CHARGING_POWER = new ReadRegistersHandler(0x0109, 1);
	@Override public Integer getChargingPower() {
		return oneRegister(CHARGING_POWER);
	}
	
	private static MessageHandler<int[]> DAILY_MIN_BATTERY_VOLTAGE = new ReadRegistersHandler(0x010B, 1);
	@Override public float getDailyMinBatteryVoltage() {
		return oneRegister(DAILY_MIN_BATTERY_VOLTAGE) / 10.0F;
	}
	private static MessageHandler<int[]> DAILY_MAX_BATTERY_VOLTAGE = new ReadRegistersHandler(0x010C, 1);
	@Override public float getDailyMaxBatteryVoltage() {
		return oneRegister(DAILY_MAX_BATTERY_VOLTAGE) / 10.0F;
	}
	
	private static MessageHandler<int[]> DAILY_MAX_CHARGING_CURRENT = new ReadRegistersHandler(0x010D, 1);
	@Override public float getDailyMaxChargingCurrent() {
		return oneRegister(DAILY_MAX_CHARGING_CURRENT) / 100.0F;
	}
	private static MessageHandler<int[]> DAILY_MAX_DISCHARGING_CURRENT = new ReadRegistersHandler(0x010E, 1);
	@Override
	public float getDailyMaxDischargingCurrent() {
		return oneRegister(DAILY_MAX_DISCHARGING_CURRENT) / 100.0F;
	}
	private static MessageHandler<int[]> DAILY_MAX_CHARGING_POWER = new ReadRegistersHandler(0x010F, 1);
	@Override public int getDailyMaxChargingPower() {
		return oneRegister(DAILY_MAX_CHARGING_POWER);
	}
	private static MessageHandler<int[]> DAILY_MAX_DISCHARGING_POWER = new ReadRegistersHandler(0x0110, 1);
	@Override public int getDailyMaxDischargingPower() {
		return oneRegister(DAILY_MAX_DISCHARGING_POWER);
	}
	
	private static MessageHandler<int[]> DAILY_AH_CHARGING = new ReadRegistersHandler(0x0111, 1);
	@Override public int getDailyAH() {
		return oneRegister(DAILY_AH_CHARGING);
	}
	private static MessageHandler<int[]> DAILY_AH_DISCHARGING = new ReadRegistersHandler(0x0112, 1);
	@Override public int getDailyAHDischarging() {
		return oneRegister(DAILY_AH_DISCHARGING);
	}
	
	private static MessageHandler<int[]> DAILY_KWH_CHARGING = new ReadRegistersHandler(0x0113, 1);
	@Override public float getDailyKWH() {
		return oneRegister(DAILY_KWH_CHARGING) / KWH_DIVIDER;
	}
	
	private static MessageHandler<int[]> DAILY_KWH_DISCHARGING = new ReadRegistersHandler(0x0114, 1);
	@Override public float getDailyKWHConsumption() {
		return oneRegister(DAILY_KWH_DISCHARGING) / KWH_DIVIDER;
	}
	
	private static MessageHandler<int[]> OPERATING_DAYS = new ReadRegistersHandler(0x0115, 1);
	@Override public int getOperatingDaysCount() {
		return oneRegister(OPERATING_DAYS);
	}
	private static MessageHandler<int[]> OVER_DISCHARGE_COUNT = new ReadRegistersHandler(0x0116, 1);
	@Override public int getBatteryOverDischargesCount() {
		return oneRegister(OVER_DISCHARGE_COUNT);
	}
	private static MessageHandler<int[]> FULL_CHARGE_COUNT = new ReadRegistersHandler(0x0117, 1);
	@Override public int getBatteryFullChargesCount() {
		return oneRegister(FULL_CHARGE_COUNT);
	}
	
	private static MessageHandler<int[]> AH_CHARGING_COUNT = new ReadRegistersHandler(0x0118, 2);
	@Override public int getChargingAmpHoursOfBatteryCount() {
		return twoRegistersAsInt(AH_CHARGING_COUNT);
	}
	private static MessageHandler<int[]> AH_DISCHARGING_COUNT = new ReadRegistersHandler(0x011A, 2);
	@Override public int getDischargingAmpHoursOfBatteryCount() {
		return twoRegistersAsInt(AH_DISCHARGING_COUNT);
	}
	
	private static MessageHandler<int[]> CUMULATIVE_KWH_CHARGING = new ReadRegistersHandler(0x011C, 2);
	@Override public float getCumulativeKWH() {
		return twoRegistersAsInt(CUMULATIVE_KWH_CHARGING) / KWH_DIVIDER;
	}
	private static MessageHandler<int[]> CUMULATIVE_KWH_DISCHARGING = new ReadRegistersHandler(0x011E, 2);
	@Override public float getCumulativeKWHConsumption() {
		return twoRegistersAsInt(CUMULATIVE_KWH_DISCHARGING) / KWH_DIVIDER;
	}
	
	private static MessageHandler<int[]> STREET_LIGHT_AND_CHARGING_STATE = new ReadRegistersHandler(0x0120, 2);
	...
	private static MessageHandler<int[]> ERROR_MODE = new ReadRegistersHandler(0x0121, 2);
	@Override public int getErrorMode() {
		return twoRegistersAsInt(ERROR_MODE);
	}
	
	private static MessageHandler<int[]> NOMINAL_BATTERY_CAPACITY = new ReadRegistersHandler(0xE002, 1);
	@Override public int getNominalBatteryCapacity() {
		return oneRegister(NOMINAL_BATTERY_CAPACITY);
	}
	
	private static MessageHandler<int[]> SYSTEM_AND_RECOGNIZED_VOLTAGE = new ReadRegistersHandler(0xE003, 1);
	...
	private static MessageHandler<int[]> BATTERY_TYPE = new ReadRegistersHandler(0xE004, 1);
	@Override public int getBatteryTypeValue() {
		return oneRegister(BATTERY_TYPE);
	}
	
	private static MessageHandler<int[]> OVER_VOLTAGE_THRESHOLD = new ReadRegistersHandler(0xE005, 1);
	@Override public int getOverVoltageThresholdRaw() {
		return oneRegister(OVER_VOLTAGE_THRESHOLD);
	}
	
	private static MessageHandler<int[]> CHARGING_VOLTAGE_LIMIT = new ReadRegistersHandler(0xE006, 1);
	@Override public int getChargingVoltageLimitRaw() {
		return oneRegister(CHARGING_VOLTAGE_LIMIT);
	}
	
	private static MessageHandler<int[]> EQUALIZING_CHARGING_VOLTAGE = new ReadRegistersHandler(0xE007, 1);
	@Override public int getEqualizingChargingVoltageRaw() {
		return oneRegister(EQUALIZING_CHARGING_VOLTAGE);
	}
	
	private static MessageHandler<int[]> BOOST_CHARGING_VOLTAGE = new ReadRegistersHandler(0xE008, 1);
	@Override public int getBoostChargingVoltageRaw() {
		return oneRegister(BOOST_CHARGING_VOLTAGE);
	}
	
	private static MessageHandler<int[]> FLOATING_CHARGING_VOLTAGE = new ReadRegistersHandler(0xE009, 1);
	@Override public int getFloatingChargingVoltageRaw() {
		return oneRegister(FLOATING_CHARGING_VOLTAGE);
	}
	
	private static MessageHandler<int[]> BOOST_CHARGING_RECOVERY_VOLTAGE = new ReadRegistersHandler(0xE00A, 1);
	@Override public int getBoostChargingRecoveryVoltageRaw() {
		return oneRegister(BOOST_CHARGING_RECOVERY_VOLTAGE);
	}
	
	private static MessageHandler<int[]> OVER_DISCHARGE_RECOVERY_VOLTAGE = new ReadRegistersHandler(0xE00B, 1);
	@Override public int getOverDischargeRecoveryVoltageRaw() {
		return oneRegister(OVER_DISCHARGE_RECOVERY_VOLTAGE);
	}
	
	private static MessageHandler<int[]> UNDER_VOLTAGE_WARNING_LEVEL = new ReadRegistersHandler(0xE00C, 1);
	@Override public int getUnderVoltageWarningLevelRaw() {
		return oneRegister(UNDER_VOLTAGE_WARNING_LEVEL);
	}
	
	private static MessageHandler<int[]> OVER_DISCHARGE_VOLTAGE = new ReadRegistersHandler(0xE00D, 1);
	@Override public int getOverDischargeVoltageRaw() {
		return oneRegister(OVER_DISCHARGE_VOLTAGE);
	}
	
	private static MessageHandler<int[]> DISCHARGING_LIMIT_VOLTAGE = new ReadRegistersHandler(0xE00E, 1);
	@Override public int getDischargingLimitVoltageRaw() {
		return oneRegister(DISCHARGING_LIMIT_VOLTAGE);
	}
	
	private static MessageHandler<int[]> END_OF_CHARGE_AND_DISCHARGE_SOC = new ReadRegistersHandler(0xE00F, 1);
	...
	private static MessageHandler<int[]> OVER_DISCHARGE_TIME_DELAY = new ReadRegistersHandler(0xE010, 1);
	@Override public int getOverDischargeTimeDelaySeconds() {
		return oneRegister(OVER_DISCHARGE_TIME_DELAY);
	}
	
	private static MessageHandler<int[]> EQUALIZING_CHARGING_TIME = new ReadRegistersHandler(0xE011, 1);
	@Override public int getEqualizingChargingTimeRaw() {
		return oneRegister(EQUALIZING_CHARGING_TIME);
	}
	
	private static MessageHandler<int[]> BOOST_CHARGING_TIME = new ReadRegistersHandler(0xE012, 1);
	@Override public int getBoostChargingTimeRaw() {
		return oneRegister(BOOST_CHARGING_TIME);
	}
	
	private static MessageHandler<int[]> EQUALIZING_CHARGING_INTERVAL = new ReadRegistersHandler(0xE013, 1);
	@Override public int getEqualizingChargingIntervalRaw() {
		return oneRegister(EQUALIZING_CHARGING_INTERVAL);
	}
	
	private static MessageHandler<int[]> TEMPERATURE_COMPENSATION = new ReadRegistersHandler(0xE014, 1);
	...
	private static MessageHandler<int[]> LOAD_WORKING_MODE = new ReadRegistersHandler(0xE01D, 1);
	@Override public int getLoadWorkingModeValue() {
		return oneRegister(LOAD_WORKING_MODE);
	}
	
	private static MessageHandler<int[]> LIGHT_CONTROL_DELAY = new ReadRegistersHandler(0xE01E, 1);
	@Override public int getLightControlDelayMinutes() {
		return oneRegister(LIGHT_CONTROL_DELAY);
	}
	
	private static MessageHandler<int[]> LIGHT_CONTROL_VOLTAGE = new ReadRegistersHandler(0xE01F, 1);
	@Override public int getLightControlVoltage() {
		return oneRegister(LIGHT_CONTROL_VOLTAGE);
	}
	
	private static MessageHandler<int[]> LED_LOAD_CURRENT_SETTING = new ReadRegistersHandler(0xE020, 1);
	@Override public int getLEDLoadCurrentSettingRaw() {
		return oneRegister(LED_LOAD_CURRENT_SETTING);
	}
	
	private static MessageHandler<int[]> SPECIAL_POWER_CONTROL_E021 = new ReadRegistersHandler(0xE021, 1);
	...
	@Override public int getWorkingHoursRaw(Sensing sensing) {
		return oneRegister(new ReadRegistersHandler(sensing.getWorkingHoursRegister(), 1));
	}
	@Override public int getPowerWithPeopleSensedRaw(Sensing sensing) {
		return oneRegister(new ReadRegistersHandler(sensing.getPowerWithPeopleSensedRegister(), 1));
	}
	...
	private static MessageHandler<int[]> SENSING_TIME_DELAY = new ReadRegistersHandler(0xE02B, 1);
	@Override public int getSensingTimeDelayRaw() {
		return oneRegister(SENSING_TIME_DELAY);
	}
	private static MessageHandler<int[]> LED_LOAD_CURRENT = new ReadRegistersHandler(0xE02C, 1);
	@Override public int getLEDLoadCurrentRaw() {
		return oneRegister(LED_LOAD_CURRENT);
	}
	private static MessageHandler<int[]> SPECIAL_POWER_CONTROL_E02D = new ReadRegistersHandler(0xE02D, 1);
