...
public class RoverModbusSlaveRead extends AbstractModbusRead implements RoverReadTable {
	...
	// region 0x000s
	static final MessageHandler<int[]> MAX_VOLTAGE_AND_CHARGING = new ReadHoldingRegisters(0x000A, 1);
	@Override
	...
	static final MessageHandler<int[]> DISCHARGING_AND_PRODUCT_TYPE = new ReadHoldingRegisters(0x000B, 1);
	...
	static final MessageHandler<int[]> PRODUCT_MODEL = new ReadHoldingRegisters(0x000C, 8);
	...
	static final MessageHandler<int[]> SOFTWARE_VERSION = new ReadHoldingRegisters(0x0014, 2);
	@Override public int getSoftwareVersionValue() {
		return twoRegistersAsInt(SOFTWARE_VERSION);
	}
	static final MessageHandler<int[]> HARDWARE_VERSION = new ReadHoldingRegisters(0x0016, 2);
	@Override public int getHardwareVersionValue() {
		return twoRegistersAsInt(HARDWARE_VERSION);
	}
	static final MessageHandler<int[]> PRODUCT_SERIAL = new ReadHoldingRegisters(0x0018, 2);
	@Override public int getProductSerialNumber() {
		return twoRegistersAsInt(PRODUCT_SERIAL);
	}

	static final MessageHandler<int[]> ADDRESS = new ReadHoldingRegisters(0x001A, 1);
	@Override
	public int getControllerDeviceAddress() {
		return oneRegister(ADDRESS);
	}

	static final MessageHandler<int[]> PROTOCOL_VERSION = new ReadHoldingRegisters(0x001B, 2);
	@Override
	public @Nullable Integer getProtocolVersionValue() {
		return twoRegistersAsInt(PROTOCOL_VERSION);
	}

	static final MessageHandler<int[]> ID_CODE = new ReadHoldingRegisters(0x001D, 2);
	@Override
	...
	// region 0x0100s
	static final MessageHandler<int[]> SOC = new ReadHoldingRegisters(0x0100, 1);
	@Override
	public int getBatteryCapacitySOC() {
		return oneRegister(SOC);
	}

	static final MessageHandler<int[]> BATTERY_VOLTAGE = new ReadHoldingRegisters(0x0101, 1);
	@Override
	public float getBatteryVoltage() {
		return oneRegister(BATTERY_VOLTAGE) / 10.0F;
	}

	static final MessageHandler<int[]> CHARGING_CURRENT = new ReadHoldingRegisters(0x0102, 1);
	@Override
	...
	static final MessageHandler<int[]> CONTROLLER_BATTERY_TEMPERATURE = new ReadHoldingRegisters(0x0103, 1);
	...
	static final MessageHandler<int[]> LOAD_VOLTAGE = new ReadHoldingRegisters(0x0104, 1);
	@Override
	public float getLoadVoltageRaw() {
		return oneRegister(LOAD_VOLTAGE) / 10.0F;
	}

	static final MessageHandler<int[]> LOAD_CURRENT = new ReadHoldingRegisters(0x0105, 1);
	@Override public float getLoadCurrentRaw() {
		return oneRegister(LOAD_CURRENT) / 100.0F;
	}

	static final MessageHandler<int[]> LOAD_POWER = new ReadHoldingRegisters(0x0106, 1);
	@Override public int getLoadPowerRaw() {
		return oneRegister(LOAD_POWER);
	}

	static final MessageHandler<int[]> PV_VOLTAGE = new ReadHoldingRegisters(0x0107, 1);
	@NotNull
	@Override public Float getPVVoltage() { // pv voltage/solar panel voltage
		return oneRegister(PV_VOLTAGE) / 10.0F;
	}
	static final MessageHandler<int[]> PV_CURRENT = new ReadHoldingRegisters(0x0108, 1);
	@NotNull
	@Override public Float getPVCurrent() {
		return oneRegister(PV_CURRENT) / 100.0F;
	}

	static final MessageHandler<int[]> CHARGING_POWER = new ReadHoldingRegisters(0x0109, 1);
	...
	static final MessageHandler<int[]> DAILY_MIN_BATTERY_VOLTAGE = new ReadHoldingRegisters(0x010B, 1);
	@Override public float getDailyMinBatteryVoltage() {
		return oneRegister(DAILY_MIN_BATTERY_VOLTAGE) / 10.0F;
	}
	static final MessageHandler<int[]> DAILY_MAX_BATTERY_VOLTAGE = new ReadHoldingRegisters(0x010C, 1);
	@Override public float getDailyMaxBatteryVoltage() {
		return oneRegister(DAILY_MAX_BATTERY_VOLTAGE) / 10.0F;
	}

	static final MessageHandler<int[]> DAILY_MAX_CHARGING_CURRENT = new ReadHoldingRegisters(0x010D, 1);
	@Override public float getDailyMaxChargingCurrent() {
		return oneRegister(DAILY_MAX_CHARGING_CURRENT) / 100.0F;
	}
	static final MessageHandler<int[]> DAILY_MAX_DISCHARGING_CURRENT = new ReadHoldingRegisters(0x010E, 1);
	@Override
	public float getDailyMaxDischargingCurrent() {
		return oneRegister(DAILY_MAX_DISCHARGING_CURRENT) / 100.0F;
	}
	static final MessageHandler<int[]> DAILY_MAX_CHARGING_POWER = new ReadHoldingRegisters(0x010F, 1);
	@Override public int getDailyMaxChargingPower() {
		return oneRegister(DAILY_MAX_CHARGING_POWER);
	}
	static final MessageHandler<int[]> DAILY_MAX_DISCHARGING_POWER = new ReadHoldingRegisters(0x0110, 1);
	@Override public int getDailyMaxDischargingPower() {
		return oneRegister(DAILY_MAX_DISCHARGING_POWER);
	}

	static final MessageHandler<int[]> DAILY_AH_CHARGING = new ReadHoldingRegisters(0x0111, 1);
	@Override public int getDailyAH() {
		return oneRegister(DAILY_AH_CHARGING);
	}
	static final MessageHandler<int[]> DAILY_AH_DISCHARGING = new ReadHoldingRegisters(0x0112, 1);
	@Override public int getDailyAHDischarging() {
		return oneRegister(DAILY_AH_DISCHARGING);
	}

	static final MessageHandler<int[]> DAILY_KWH_CHARGING = new ReadHoldingRegisters(0x0113, 1);
	@Override public float getDailyKWH() {
		return oneRegister(DAILY_KWH_CHARGING) / KWH_DIVIDER;
	}

	static final MessageHandler<int[]> DAILY_KWH_DISCHARGING = new ReadHoldingRegisters(0x0114, 1);
	@Override public float getDailyKWHConsumption() {
		return oneRegister(DAILY_KWH_DISCHARGING) / KWH_DIVIDER;
	}

	static final MessageHandler<int[]> OPERATING_DAYS = new ReadHoldingRegisters(0x0115, 1);
	@Override public int getOperatingDaysCount() {
		return oneRegister(OPERATING_DAYS);
	}
	static final MessageHandler<int[]> OVER_DISCHARGE_COUNT = new ReadHoldingRegisters(0x0116, 1);
	@Override public int getBatteryOverDischargesCount() {
		return oneRegister(OVER_DISCHARGE_COUNT);
	}
	static final MessageHandler<int[]> FULL_CHARGE_COUNT = new ReadHoldingRegisters(0x0117, 1);
	@Override public int getBatteryFullChargesCount() {
		return oneRegister(FULL_CHARGE_COUNT);
	}

	static final MessageHandler<int[]> AH_CHARGING_COUNT = new ReadHoldingRegisters(0x0118, 2);
	@Override public int getChargingAmpHoursOfBatteryCount() {
		return twoRegistersAsInt(AH_CHARGING_COUNT);
	}
	static final MessageHandler<int[]> AH_DISCHARGING_COUNT = new ReadHoldingRegisters(0x011A, 2);
	@Override public int getDischargingAmpHoursOfBatteryCount() {
		return twoRegistersAsInt(AH_DISCHARGING_COUNT);
	}

	static final MessageHandler<int[]> CUMULATIVE_KWH_CHARGING = new ReadHoldingRegisters(0x011C, 2);
	@Override public float getCumulativeKWH() {
		return twoRegistersAsInt(CUMULATIVE_KWH_CHARGING) / KWH_DIVIDER;
	}
	static final MessageHandler<int[]> CUMULATIVE_KWH_DISCHARGING = new ReadHoldingRegisters(0x011E, 2);
	@Override public float getCumulativeKWHConsumption() {
		return twoRegistersAsInt(CUMULATIVE_KWH_DISCHARGING) / KWH_DIVIDER;
	}

	static final MessageHandler<int[]> STREET_LIGHT_AND_CHARGING_STATE = new ReadHoldingRegisters(0x0120, 1);
	...
	static final MessageHandler<int[]> ERROR_MODE = new ReadHoldingRegisters(0x0121, 2);
	...
	// region 0xE000s
	static final MessageHandler<int[]> CHARGING_CURRENT_SETTING = new ReadHoldingRegisters(0xE001, 1);
	...
	static final MessageHandler<int[]> NOMINAL_BATTERY_CAPACITY = new ReadHoldingRegisters(0xE002, 1);
	@Override public int getNominalBatteryCapacity() {
		return oneRegister(NOMINAL_BATTERY_CAPACITY);
	}

	static final MessageHandler<int[]> SYSTEM_AND_RECOGNIZED_VOLTAGE = new ReadHoldingRegisters(0xE003, 1);
	...
	static final MessageHandler<int[]> BATTERY_TYPE = new ReadHoldingRegisters(0xE004, 1);
	@Override public int getBatteryTypeValue() {
		return oneRegister(BATTERY_TYPE);
	}

	static final MessageHandler<int[]> OVER_VOLTAGE_THRESHOLD = new ReadHoldingRegisters(0xE005, 1);
	@Override public int getOverVoltageThresholdRaw() {
		return oneRegister(OVER_VOLTAGE_THRESHOLD);
	}

	static final MessageHandler<int[]> CHARGING_VOLTAGE_LIMIT = new ReadHoldingRegisters(0xE006, 1);
	@Override public int getChargingVoltageLimitRaw() {
		return oneRegister(CHARGING_VOLTAGE_LIMIT);
	}

	static final MessageHandler<int[]> EQUALIZING_CHARGING_VOLTAGE = new ReadHoldingRegisters(0xE007, 1);
	@Override public int getEqualizingChargingVoltageRaw() {
		return oneRegister(EQUALIZING_CHARGING_VOLTAGE);
	}

	static final MessageHandler<int[]> BOOST_CHARGING_VOLTAGE = new ReadHoldingRegisters(0xE008, 1);
	@Override public int getBoostChargingVoltageRaw() {
		return oneRegister(BOOST_CHARGING_VOLTAGE);
	}

	static final MessageHandler<int[]> FLOATING_CHARGING_VOLTAGE = new ReadHoldingRegisters(0xE009, 1);
	@Override public int getFloatingChargingVoltageRaw() {
		return oneRegister(FLOATING_CHARGING_VOLTAGE);
	}

	static final MessageHandler<int[]> BOOST_CHARGING_RECOVERY_VOLTAGE = new ReadHoldingRegisters(0xE00A, 1);
	@Override public int getBoostChargingRecoveryVoltageRaw() {
		return oneRegister(BOOST_CHARGING_RECOVERY_VOLTAGE);
	}

	static final MessageHandler<int[]> OVER_DISCHARGE_RECOVERY_VOLTAGE = new ReadHoldingRegisters(0xE00B, 1);
	@Override public int getOverDischargeRecoveryVoltageRaw() {
		return oneRegister(OVER_DISCHARGE_RECOVERY_VOLTAGE);
	}

	static final MessageHandler<int[]> UNDER_VOLTAGE_WARNING_LEVEL = new ReadHoldingRegisters(0xE00C, 1);
	@Override public int getUnderVoltageWarningLevelRaw() {
		return oneRegister(UNDER_VOLTAGE_WARNING_LEVEL);
	}

	static final MessageHandler<int[]> OVER_DISCHARGE_VOLTAGE = new ReadHoldingRegisters(0xE00D, 1);
	@Override public int getOverDischargeVoltageRaw() {
		return oneRegister(OVER_DISCHARGE_VOLTAGE);
	}

	static final MessageHandler<int[]> DISCHARGING_LIMIT_VOLTAGE = new ReadHoldingRegisters(0xE00E, 1);
	@Override public int getDischargingLimitVoltageRaw() {
		return oneRegister(DISCHARGING_LIMIT_VOLTAGE);
	}

	static final MessageHandler<int[]> END_OF_CHARGE_AND_DISCHARGE_SOC = new ReadHoldingRegisters(0xE00F, 1);
	...
	static final MessageHandler<int[]> OVER_DISCHARGE_TIME_DELAY = new ReadHoldingRegisters(0xE010, 1);
	@Override public int getOverDischargeTimeDelaySeconds() {
		return oneRegister(OVER_DISCHARGE_TIME_DELAY);
	}

	static final MessageHandler<int[]> EQUALIZING_CHARGING_TIME = new ReadHoldingRegisters(0xE011, 1);
	@Override public int getEqualizingChargingTimeRaw() {
		return oneRegister(EQUALIZING_CHARGING_TIME);
	}

	static final MessageHandler<int[]> BOOST_CHARGING_TIME = new ReadHoldingRegisters(0xE012, 1);
	@Override public int getBoostChargingTimeRaw() {
		return oneRegister(BOOST_CHARGING_TIME);
	}

	static final MessageHandler<int[]> EQUALIZING_CHARGING_INTERVAL = new ReadHoldingRegisters(0xE013, 1);
	@Override public int getEqualizingChargingIntervalRaw() {
		return oneRegister(EQUALIZING_CHARGING_INTERVAL);
	}

	static final MessageHandler<int[]> TEMPERATURE_COMPENSATION = new ReadHoldingRegisters(0xE014, 1);
	...
	static final MessageHandler<int[]> LOAD_WORKING_MODE = new ReadHoldingRegisters(0xE01D, 1);
	@Override public int getLoadWorkingModeValue() {
		return oneRegister(LOAD_WORKING_MODE);
	}

	static final MessageHandler<int[]> LIGHT_CONTROL_DELAY = new ReadHoldingRegisters(0xE01E, 1);
	@Override public int getLightControlDelayMinutes() {
		return oneRegister(LIGHT_CONTROL_DELAY);
	}

	static final MessageHandler<int[]> LIGHT_CONTROL_VOLTAGE = new ReadHoldingRegisters(0xE01F, 1);
	@Override public int getLightControlVoltage() {
		return oneRegister(LIGHT_CONTROL_VOLTAGE);
	}

	static final MessageHandler<int[]> LED_LOAD_CURRENT_SETTING = new ReadHoldingRegisters(0xE020, 1);
	@Override public int getLEDLoadCurrentSettingRaw() {
		return oneRegister(LED_LOAD_CURRENT_SETTING);
	}

	static final MessageHandler<int[]> SPECIAL_POWER_CONTROL_E021 = new ReadHoldingRegisters(0xE021, 1);
	...
	static final MessageHandler<int[]> SENSING_TIME_DELAY = new ReadHoldingRegisters(0xE02B, 1);
	...
	static final MessageHandler<int[]> LED_LOAD_CURRENT = new ReadHoldingRegisters(0xE02C, 1);
	...
	static final MessageHandler<int[]> SPECIAL_POWER_CONTROL_E02D = new ReadHoldingRegisters(0xE02D, 1);
	...
	static final MessageHandler<int[]> CONTROLLER_CHARGING_POWER_SETTING = new ReadHoldingRegisters(0xE02E, 1);
	@Override
	public Integer getControllerChargingPowerSetting() {
		return oneRegister(CONTROLLER_CHARGING_POWER_SETTING);
	}

	static final MessageHandler<int[]> GENERATOR_CHARGING_POWER_SETTING = new ReadHoldingRegisters(0xE02F, 1);
	@Override
