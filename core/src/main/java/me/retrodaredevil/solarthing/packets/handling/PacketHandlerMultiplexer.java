...
/**
 * Calls {@link PacketHandler#handle(PacketCollection, boolean)} for each {@link PacketHandler}. If a {@link PacketHandler}
 * throws a {@link PacketHandleException} or any exception for that matter, it will not call {@link PacketHandler#handle(PacketCollection, boolean)}
...
public class PacketHandlerMultiplexer implements PacketHandler {
	...
	public void handle(PacketCollection packetCollection, boolean wasInstant) throws PacketHandleException {
		for(PacketHandler handler : packetHandlers){
			handler.handle(packetCollection, wasInstant);
		}
	}
}
